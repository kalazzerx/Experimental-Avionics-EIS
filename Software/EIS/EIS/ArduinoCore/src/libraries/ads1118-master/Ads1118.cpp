...
#ifdef DEBUG
    #define DEBUG_PRINT(x)  Serial2.println(x)
#else
...
word Ads1118::update_config(uint16_t new_config)
{
    ...
    Serial2.print("Current_config is:");
    Serial2.println(CURRENT_CONFIG, HEX);
#endif
	// Wait for another sample cycle
	delay(20);
    return readConfig;
}
...
#ifdef DEBUG
    Serial2.print("Current config is: ");
    Serial2.println(CURRENT_CONFIG, HEX);
    Serial2.print("Updating configuration to: ");
    Serial2.println(port | (CURRENT_CONFIG & ~(PIN_BITMASK)), HEX);
    Serial2.print("CURRENT_CONFIG & ~PIN_BITMASK = ");
    Serial2.println((CURRENT_CONFIG & ~PIN_BITMASK), HEX);
#endif
...
#ifdef DEBUG
    Serial2.print("Input mux: 0x");
    Serial2.println(PIN_BITMASK & CURRENT_CONFIG, HEX);
    Serial2.print("MOSI: ");
    Serial2.println(CURRENT_CONFIG, HEX);
#endif
...
double Ads1118::readTemp()
{
    ...
    */
    ...
    Serial2.println((CURRENT_CONFIG >> 8), HEX);
#endif
    MSB = SPI.transfer((CURRENT_CONFIG >> 8) & 0xFF);
#ifdef DEBUG
    Serial2.print("Sending: ");
    Serial2.println((CURRENT_CONFIG & 0xFF), HEX);
#endif
    LSB = SPI.transfer(CURRENT_CONFIG & 0xFF);
    ...
    read = (MSB << 6) | (LSB >> 2);
#ifdef DEBUG
    Serial2.print("Raw MSB =");
    Serial2.println(MSB, BIN);
    Serial2.print("Raw LSB =");
    Serial2.println(LSB, BIN);
    Serial2.print("Raw read =");
    Serial2.println(read, BIN);
#endif
    if((0x8000 & read) == 0){
